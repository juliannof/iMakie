...
PositionController* positionController; // <-- Asegurarse que se declare
...
void setup() {
    ...
    // 1. INSTANCIAMOS MotorController PRIMERO
    motor = new MotorController();
    Serial.println(">>> PASO 1: MotorController creado.");
    delay(100);
    motor->begin(); // Llama al begin() a través del puntero
    Serial.println(">>> PASO 1.1: MotorController inicializado.");
    delay(100);

    // 2. INSTANCIAMOS FaderSensor
    sensor = new FaderSensor();
    Serial.println(">>> PASO 2: FaderSensor creado.");
    delay(100);
    sensor->begin();
    Serial.println(">>> PASO 2.1: FaderSensor inicializado.");
    delay(100);

    // 3. INSTANCIAMOS CalibrationManager
    calibrador = new CalibrationManager();
    Serial.println(">>> PASO 3: CalibrationManager creado.");
    delay(100);
    calibrador->comenzar(); // Llama a comenzar() a través del puntero
    Serial.println(">>> PASO 3.1: CalibrationManager inicializado.");
    delay(100);

    // 4. INSTANCIAMOS PositionController, pasándole AMBOS punteros
    //    Ahora, el 'motor' ya está creado y podemos pasarlo.
    positionController = new PositionController(sensor, motor); // <-- ¡CORREGIDO AQUÍ!
    Serial.println(">>> PASO 5: PositionController creado.");
    delay(100);
