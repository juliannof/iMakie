...
void setup() {
    ...
    indicador = new StatusManager();
    Serial.println(">>> PASO 4: StatusManager creado."); // Debug point
    delay(100);
    indicador->begin();
    Serial.println(">>> PASO 4.1: StatusManager inicializado."); // Debug point
    delay(100);
...
void manejarCalibracion() {
    if (calibrador->estaCalibrando()) {
        ...
        if (calibrador->obtenerEstado() == CalibrationManager::COMPLETADO) {
            ...
            // Si el calibrador está Inactivo, y no se está calibrando y no completó, resetear la bandera.
