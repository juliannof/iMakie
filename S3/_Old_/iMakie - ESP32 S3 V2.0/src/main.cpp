...
bool vuClipState[8] = {false};
unsigned long vuLastUpdateTime[8] = {0}; // Timestamp de la última vez que el nivel de VU fue actualizado
float vuPeakLevels[8] = {0.0f};             // Nivel de pico
unsigned long vuPeakLastUpdateTime[8] = {0}; // Timestamp de la última vez que el pico de VU fue actualizado

// String para TimeCode / Beats (usado para display, actualizado por MIDI)
...
void loop() {
    ...
    // Actualiza el string del tempo si projectTempo ha cambiado
    static float lastProjectTempo = 0.0f;
    if (projectTempo != lastProjectTempo) {
        snprintf(tempoString, sizeof(tempoString), "%.2f BPM", projectTempo);
        lastProjectTempo = projectTempo;
        needsHeaderRedraw = true; // El tempo afecta al HEADER, activar redibujo específico
        log_d("Tempo actualizado a: %s", tempoString);
    }
    ...
    if (logicConnectionState == ConnectionState::CONNECTED) {
        if (!wasConnected) { // Transición de no conectado a conectado
            ...
            log_i("Conectado a DAW. Forzando redibujo completo.");
        }
        ...
        // updateDisplay() se llama al final del loop para manejar el redibujo
