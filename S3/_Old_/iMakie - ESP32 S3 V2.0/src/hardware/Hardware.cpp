...
void initHardware() {
  Serial.println("[SETUP] Iniciando NeoTrellis...");
  if (!trellis.begin()) {
    Serial.println("!!! ERROR FATAL: NeoTrellis no encontrado. Comprueba I2C.");
    pinMode(LED_BUILTIN, OUTPUT);
    while (1) { digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN)); delay(100); }
  }
  Serial.println("        -> Trellis.begin() exitoso.");

  Serial.println("[SETUP] Registrando callbacks...");
  ...
  // Apagar todos los LEDs al inicio como estado base
  for (int i = 0; i < X_DIM * Y_DIM; i++) {
...
void setTrellisBrightness(uint8_t newBrightness) {
  ...
  Serial.printf("[HARDWARE] Brillo de Trellis ajustado a: %d\n", newBrightness);
