...
// ****************************************************************************
void processMidiByte(byte b) {
    if (b >= 0xF8) { // Filtra mensajes de Tiempo Real (F8-FF)
        // No loguear si no se procesa, para evitar ruido excesivo.
        return;
    }
    ...
    if (handshakeState == HandshakeState::AWAITING_CHALLENGE_BYTES) {
        ...
        if (b < 0x80) { // ¿Es un byte de datos?
            if (challenge_idx < 7) {
                ...
                log_w("[HANDSHAKE-PESCA] Advertencia: challenge_idx fuera de límites (%d) durante pesca de desafío. Byte 0x%02X.", challenge_idx, b);
            }
...
// src/midi/MIDIProcessor.cpp (ubicada en su archivo .cpp)
...
void processControlChange(byte channel, byte controller, byte value) {
    ...
    // El Mackie Display usa Channel 0 o 15 para CCs, y controladores del 64 al 73 (0x40 a 0x49).
    if ((channel != 0 && channel != 15) || (controller < 64 || controller > 73)) {
        log_v("processControlChange: CC ignorado (no es para Mackie Display). Channel=%d, Controller=%d.", channel, controller);
        return;
    }
    
    // El Mackie control usa los controladores del 64 al 73 para los dígitos del display,
    // donde 73 es el dígito 0 (más a la derecha) y 64 es el dígito 9 (más a la izquierda).
    int digit_index = 73 - controller; // Calcula el índice del dígito (0-9)
    
    log_v("processControlChange: Decodificando para digit_index=%d (Controller=%d).", digit_index, controller);

    // Verificación de límites para el índice del dígito
    if (digit_index < 0 || digit_index >= 10) { 
        log_e("processControlChange: ERROR: Índice de dígito calculado fuera de rango (0-9): %d para CC %d.", digit_index, controller);
        return; // Retornar si el índice no es válido
    }
    
    // Extraer el código del carácter Mackie (bits 0-5 de 'value')
    byte char_code = value & 0x3F; // 0x3F = bin 00111111, máscara para los 6 bits bajos
    char display_char;

    // Buscar el carácter ASCII correspondiente en el mapa
    if (char_code < 64) { 
        display_char = MACKIE_CHAR_MAP[char_code];
        log_v("processControlChange: char_code=0x%02X (%d), Mapeado a ASCII '%c'.", char_code, char_code, display_char);
    } else {
        display_char = '?'; // Si el char_code no está en la tabla (debería ser < 64)
        log_w("processControlChange: char_code=0x%02X (%d) fuera de rango para MACKIE_CHAR_MAP. Mostrando '?'.", char_code, char_code);
    }
    
    // Preparar el carácter para almacenar, incluyendo el bit del punto (bit 6 de 'value')
    byte char_to_store = (byte)display_char; // Empieza con el carácter ASCII
    if ((value & 0x40) != 0) { // Si el bit 6 de 'value' está activo... (0x40 = bin 01000000)
        char_to_store |= 0x80; // ...activar el bit MSB en 'char_to_store' para indicar un punto
        log_v("processControlChange: Bit 6 activo (0x40 en value), indicando un punto. char_to_store=0x%02X.", char_to_store);
    } else {
        log_v("processControlChange: Bit 6 inactivo, no hay punto.");
    }
    
    // --- LÍNEAS COMENTADAS TEMPORALMENTE PARA IMPEDIR ACTUALIZACIÓN DEL DISPLAY ---
    // // Actualizar el array "sucio" (beats o timecode)
    // if (currentTimecodeMode == MODE_BEATS) {
    //     log_d("processControlChange: (TEMPORALMENTE NO) Actualizando beatsChars_dirty[%d] con 0x%02X ('%c').", digit_index, char_to_store, display_char);
    //     // beatsChars_dirty[digit_index] = char_to_store;
    // } else {
    //     log_d("processControlChange: (TEMPORALMENTE NO) Actualizando timeCodeChars_dirty[%d] con 0x%02X ('%c').", digit_index, char_to_store, display_char);
    //     // timeCodeChars_dirty[digit_index] = char_to_store;
    // }
    
    // // Marcamos que hay datos nuevos y actualizamos el tiempo de la última recepción
    // log_d("processControlChange: (TEMPORALMENTE NO) Activando displayDataUpdated y lastDisplayCC_Time.");
    // // displayDataUpdated = true; 
    // // lastDisplayCC_Time = millis(); // <-- ¡CRÍTICO: Asegúrate de que esta línea esté en la versión final!
    
    log_i("processControlChange: Decodificación completa - Digit[%d], RawValue:%d, CharCode:0x%02X ('%c'), Dot:%s",
          digit_index, value, char_code, display_char, ((value & 0x40) != 0) ? "YES" : "NO");
}
...
void processChannelPressure(byte channel, byte value) {
    // 'channel' es el canal MIDI del mensaje (0-15)
    // 'value' es el byte de datos del mensaje (0-127)
    float normalizedLevel = 0.0f;
    int targetChannel = -1;
    bool newClipState = false;
    bool clearClip = false;

    // --- Detección de formato (sin cambios) ---
    if (channel == 0) {
        // FORMATO "PURO" MCU
...
void processMackieSysEx(byte* payload, int len) {
    ...
    if (command == 0x00 && len == 5) {
        Serial.println("[HANDSHAKE] 'Connection Query' (0x00) recibida. Empezando a 'pescar' 7 bytes de desafío...");
    ...
    switch(command) {
        case 0x0E: { // Version Request
            ...
            if (currentTime - lastVersionReplyTime > VERSION_REPLY_COOLDOWN_MS) {
                Serial.println("<<< Version Request recibido. Enviando UNA respuesta.");
        ...
        case 0x10: { // Time Code / BBT Display
            if (len >= 15) { 
                char time_buf[11];
        ...
        case 0x11: { // Assignment Display
            if (len >= 7) {
                char assign_buf[3];
                assign_buf[0] = payload[5];
                assign_buf[1] = payload[6];
                assign_buf[2] = '\0';
                if (assignmentString != assign_buf) assignmentString = String(assign_buf);
            }
            break;
        }
...
void processPitchBend(byte channel, int bendValue) {
    ...
    if (channel >= 0 && channel < 8) { // Cambiado a < 8 dado tu implementación para faders 0-7
