...
- **ElegantOTA WiFi — Estado residual sin reinicio equipo (2026-05-03)** — **CAUSA RAÍZ IDENTIFICADA** — Patrón fail→fail→success sin reinicio equipo causado por **Update.abort() missing**. Cuando OTA falla a mitad, partición OTA queda en estado "half-written". Siguiente intento ve esa partición inconsistente y también falla. Tercer intento: Update timeout internos resuelven estado (probabilístico). **Solución:** Agregar `Update.abort(); delay(100);` en `OtaManager::enableForUpload()` línea ~154 (justo antes de `ElegantOTA.begin(&server)`). Esto marca partición como libre en tabla, permitiendo que ElegantOTA comience con estado limpio. **Validación:** Pendiente implementar + compilar + test 5 intentos OTA sin reinicio equipo. **Documentación:** Ver `memory/s2_ota_wifi_state_residual.md` para detalles de investigación.
